...
-(void)initializeLogger
{
    if (buffer)
    {
        logger.text = @""; // clear out the "panic" text
        [logger logString:buffer withTick:""];
        buffer = nil; // flush out the log buffer once
    }
}

+ (void)sendToLog:(NSString *)string withTick:(const char *)str
{
    if (logger)
    {
        if (string != nil)
            [logger logString:[NSString stringWithFormat:@"%@%@", logger.text, string] withTick:"*"];
    }
    else
        buffer = [NSString stringWithFormat:@"%@[%s] %@\n", buffer ?: @"", str, string];
}

-(int)log:(NSString*)str
{
    if (self.logs && str)
    {
        self.logs.text = [NSString stringWithFormat:@"%@%@", self.logs.text, str];
        return 0;
    }
    return 1;
}
...
int printf(const char *format, ...)
{
    ...
    if ( logger )
    {
        ...
        va_end(argptr);
        [logger logCString:str withTick:"~"]; // log debug statements
    }
...
- (void)viewDidLoad {
    ...
    _blurOverlay.layer.cornerRadius = 15;
    if (@available(iOS 13, *))
        _blurOverlay.layer.cornerCurve = kCACornerCurveContinuous;
    _jailbreakButton.layer.cornerRadius = 5;
    if (@available(iOS 13, *))
        _jailbreakButton.layer.cornerCurve = kCACornerCurveContinuous;
    ...
    [self.jbButton addTarget:self action:@selector(jailbreak:) forControlEvents:UIControlEventTouchDown];
...
- (void)jailbreak:(id)sender {
    ...
    if (debug) {
        ...
        if (ret) {
            ...
            else if (maxVersion("13.3")) {
                taskforpidzero = time_waste();
            }
            else {
                return;
            }
    ...
    if (!debug)
    {
        LOG("[i] Compiled without debug enabled.");
        LOG("[i] Continued execution will panic the device.");
        LOG("[i] Compile with debug only if you are contributing.");
        return;
    }
    LOG("[*] Starting fun");
